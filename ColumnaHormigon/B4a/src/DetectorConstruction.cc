...
//Creo una caja para el "suelo"
  G4double downBox_hx = 11.0 * cm;
  G4double downBox_hy = 2.5 * cm;
  G4double downBox_hz = 4.5 * cm;
  
  auto solidDownBox = new G4Box("solidDownBox",
      downBox_hx, downBox_hy, downBox_hz);
  ...
  auto airBoxPV
      = new G4PVPlacement(nullptr,
          G4ThreeVector(plomo_hx-airBox_hx+1*cm, 0.0, 0.0),
          airBoxLV,
          "AirBoxPV",
          plomoLV,
          false,
  ...
  //plomoLV->SetMaterial(defaultMaterial);
 ...
 // auto airLV
  ...
  /*
    ...
    */
    ...
    G4VisAttributes* solidDownBoxVisAtt = new G4VisAttributes(G4Colour(1.0, 0.0, 1.0)); // Azul claritoB
    solidDownBoxVisAtt->SetVisibility(true);
    solidDownBoxLV->SetVisAttributes(solidDownBoxVisAtt);
