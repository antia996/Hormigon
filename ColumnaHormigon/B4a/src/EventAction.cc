...
void EventAction::EndOfEventAction(const G4Event* event)
{
    ...
    for (G4int iVertex = 0; iVertex < nPrimaries; ++iVertex) {
        ...
        if (primary->GetPDGcode() == 2112) {
            ...
            analysisManager->FillH1(1, fLastPosition.mag());

            // fill ntuple
            analysisManager->FillNtupleDColumn(0, fEnergyDetector);
            analysisManager->FillNtupleDColumn(1, fLastPosition.mag());
...
void EventAction::SetLastPosition(const G4ThreeVector& position) {
